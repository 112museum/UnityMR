    ...
    public class PhotonRoomWordPuzzle : MonoBehaviourPunCallbacks, IInRoomCallbacks
    {
        ...
        private void CreateInteractableObjects()
        {
            ...
            for (int i = 0; i < interactablePrefabs.Count; i++)
            {
                ...
                // Instantiated as a room object (creator actor number 0) so it is not tied to whichever
                // player happened to create it — it survives regardless of who leaves the room, including
                // the Master Client that created it.
                GameObject instantiatedObj = PhotonNetwork.InstantiateRoomObject(
